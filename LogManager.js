...
/**
 * @description Internal class for identifying log entries that are created by LogManager::logEntry
 */
class LogEntry {}
 ...
 *  {Object} winstonLoggers {String} category -> Winston logger
 ...
 *  {Object} winstonLevels Pass6d to Winston when creating a logger
 *
...
stage: '${options.stage}' host id: ${this.hostId}`);
    ...
    const uncaughtLogger = this.logger(options.uncaughtCategory);
    // Create a Winston logger now to catch uncaught exceptions
    if (uncaughtLogger.isLevelEnabled('error')) {
      this.unhandledPromiseListener = (error) => {
        uncaughtLogger.error('Unhandled Promise rejection', { error });
      };
  ...
  async close() {
    ...
    const errorLogger = this.winstonLoggers[cloudWatchErrorCategory];
    this.loggers = {};
   ...
   */
  createWinstonLogger(category) {
    if (this.stopped) throw new Error('Stopped');

    let logger;

    if (category === this.options.cloudWatch.errorCategory) {
      logger = this.createCloudWatchErrorLogger();
    } else {
...
'${awsOptions.region}:${logGroupName}:${this.cloudWatch.streamName}' at level '${level}'`);
  ...
  winstonLogger(category) {
    ...
    if (!logger) logger = this.createWinstonLogger(category);
      ...
      if (typeof item === 'object') {
        if (item instanceof Array) {
          ...
          Object.entries(item).forEach(([key, value]) => {
            if (key === 'tags') {
              tags = LogManager.tags(tags, value);
...
class ContextLogger {
  ...
  log(tags, message, extra, category) {
    ...
    if (info) {
      // The objective of the above code is to avoid calling LogManager.extra if the level is too low
